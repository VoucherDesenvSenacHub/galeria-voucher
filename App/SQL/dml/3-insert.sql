-- 1. INSERTS PARA TABELA 'imagem' (incluindo imagens genéricas para turmas e admin)
...
('./upload/turma-galeria.png', NOW());   -- ID 25: Imagem Padrão para Turmas

-- 2. INSERTS PARA TABELA 'turma' (com imagem_id obrigatório)
...
-- 3. INSERTS PARA TABELA 'pessoa' (alunos, professores, administradores)
...
-- 4. INSERTS PARA TABELA 'usuario' (senhas criptografadas)
...
-- 5. INSERTS PARA TABELA 'aluno_turma'
...
-- 6. INSERTS PARA TABELA 'docente_turma'
...
-- 8. Captura o ID do projeto inserido
SET @id_projeto = LAST_INSERT_ID();

-- 10. INSERIR IMAGENS BASE DO PROJETO (use `text` entre backticks porque é palavra reservada)
INSERT INTO imagem (url, data_upload) VALUES
('./upload/turma-galeria.png', NOW()),
('./upload/turma-galeria.png', NOW()),
('./upload/turma-galeria.png', NOW());

-- 9. INSERIR REGISTROS NA TABELA 'projeto_dia'
INSERT INTO projeto_dia (tipo_dia, descricao, projeto_id)
VALUES 
('I', 'Início do projeto: definição de escopo e tecnologias.', @id_projeto),
('P', 'Processo de desenvolvimento: criação de layout e funcionalidades.', @id_projeto),
('E', 'Entrega final e apresentação do projeto.', @id_projeto);

-- 10. ASSOCIAR IMAGENS AOS DIAS DO PROJETO
SET @img_inicio_id = (SELECT imagem_id FROM imagem WHERE `text` = 'Início do projeto' AND url = './upload/turma-galeria.png' ORDER BY imagem_id DESC LIMIT 1);
SET @img_desenvolvimento_id = (SELECT imagem_id FROM imagem WHERE `text` = 'Desenvolvimento' AND url = './upload/turma-galeria.png' ORDER BY imagem_id DESC LIMIT 1);
SET @img_entrega_id = (SELECT imagem_id FROM imagem WHERE `text` = 'Entrega final' AND url = './upload/turma-galeria.png' ORDER BY imagem_id DESC LIMIT 1);
